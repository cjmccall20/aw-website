...
import type { TryoutCycle } from "@/lib/types";

export function RequirementsView() {
  const db = useStore();
  const active = db.tryoutCycles.find(t => t.active);

  return (
    <section className="section-sm">
      <div className="container-content grid lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-10">
          {active ? <ActiveTryoutCard cycle={active} /> : <NoCyclesCard />}
          <WhatToExpect />
          <Eligibility />
        </div>
...
function ActiveTryoutCard({ cycle }: { cycle: TryoutCycle }) {
  return (
    <article className="card-padded bg-gradient-to-br from-maroon-50 to-cream-200 border-maroon-200">
      <div className="flex items-center gap-2 text-maroon-700">
        ...
        <p className="eyebrow text-maroon-800">Current cycle</p>
      </div>
